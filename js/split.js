(function ($) {
	$.fn.split = function (options) {
		if ($(this).hasClass('split')) {
			...
			$theSlider
				.find('> .inner-wrapper > .slides-wrapper > .slides')
				.each(function () {
					$(this)
						.find('> .slide')
						.each(function (i) {
			...
			if (options.height !== 'false') {
				$theSlider
					.find('> .inner-wrapper > .slides-wrapper')
					.css('height', options.height == 'full' ? '100%' : options.height);
			}
			...
			if (options.arrows == 'true') {
				if (!$theSlider.find('> .inner-wrapper .navigation-wrapper').length) {
					$theSlider
						.find('> .inner-wrapper > .slides-wrapper')
						.after('<div class="navigation-wrapper"></div>');
				}
				$theSlider
					.find('> .inner-wrapper > .navigation-wrapper')
					.append(
						'<a href="#" class="navigation-arrow prev"></a><a href="#" class="navigation-arrow next"></a>'
					);
			}
			if (options.infinite !== 'true') {
				$theSlider
					.find('> .inner-wrapper > .navigation-wrapper .prev')
					.addClass('disable');
			}

			//Click navigation arrow event
			$theSlider
				.find('> .inner-wrapper > .navigation-wrapper .navigation-arrow')
				.on(
					'click',
					{
						delay: options.delay,
			...
			var $slides = $theSlider.find(
				'> .inner-wrapper > .slides-wrapper > .slides'
			);
			...
			$slides.each(function () {
				...
				$thisSlide.find('> .slide').each(function (j) {
					...
					if (options.dots == 'true') {
						$currentSlide.append(
							'<a href="#" class="dot" data-attribute-dot-number="' +
								j++ +
								'"></span>'
						);
			...
			//Click dots event
			$theSlider
				.find('> .inner-wrapper .dots-wrapper .dot')
				.on('click', splitDots);
	...
	function splitArrows(event) {
		...
		$currentDots = $(this)
			.parents('.split')
			.first()
			.find('> .inner-wrapper > .dots-wrapper');
		...
		if ($buttonClicked.hasClass('prev')) {
			...
			if (event.data.infinite == 'true') {
				...
				if (
					!$currentSlide.find('> .slide.previous').last().prev().length &&
					$currentSlide.find('> .slide.previous').length
				) {
			...
			$buttonClicked.hasClass('next') &&
			$currentSlide.find('> .slide.init').last().next().length
		) {
			$currentSlide.each(function () {
				var $currentSlide = $(this);
	...
	function splitDots(event) {
		event.preventDefault();
		var $clickedDot = $(this);
		var $clickedDotNext = $clickedDot
			.parents('.split')
			.first()
			.find('> .inner-wrapper > .navigation-wrapper .next');
		var $clickedDotPrev = $clickedDot
			.parents('.split')
			.first()
			.find('> .inner-wrapper > .navigation-wrapper .prev');
	...
	$(document).keyup(function (event) {
		if ($('.split').length) {
			if (event.which == 192) {
				...
				!$('.split')
					.first()
					.find('> .inner-wrapper > .navigation-wrapper .navigation-arrow.prev')
					.hasClass('disable')
			) {
				$('.split .navigation-arrow.prev')
					.not('.split .split .navigation-arrow.prev')
					.click();
			} else if (
				event.which == 39 &&
				!$('.split')
					.first()
					.find('> .inner-wrapper > .navigation-wrapper .navigation-arrow.next')
					.hasClass('disable')
			) {
				$('.split .navigation-arrow.next')
					.not('.split .split .navigation-arrow.next')
					.click();
			}
		}
	});
			...
			splitSwipe(element, function (swipedirection) {
				if (
					...
					!$('.split')
						.first()
						.find(
							'> .inner-wrapper > .navigation-wrapper .navigation-arrow.next'
						)
						.hasClass('disable')
				) {
					$('.split .navigation-arrow.next')
					...
					!$('.split')
						.first()
						.find(
							'> .inner-wrapper > .navigation-wrapper .navigation-arrow.prev'
						)
						.hasClass('disable')
				) {
					$('.split .navigation-arrow.prev')
